...
function AfterLogin() {
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (userData.nickname!=""){
            navigate("/blurt/main", { state: { color: hexcodeChosen, name, nickname, mode } });
          }
  ...
  const handleSubmit = async () => {
    ...
    try {
      const stuff = { googleId: userId, name, nickname, color: hsvaToHex(hexcodeChosen), mode };
      console.log(stuff);
      const response = await fetch("http://localhost:3000/users/save", {
        method: "POST",
      ...
      navigate("/blurt/main", { state: { color: hsvaToHex(hexcodeChosen), name, nickname, mode } });
    } catch (error) {
